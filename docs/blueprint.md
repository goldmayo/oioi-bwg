...
- 컨셉 후보 ( 마음에 들면 UI 방식 버튼으로 전환 가능하게 만들 수 있는 것도 고려중)

1. [iPod Re-imagined] "iBawige Classic"
   UI: 화면 중앙에 2000년대 초반 iPod의 클릭 휠(Click Wheel) 인터페이스를 3D로 구현합니다.

인터랙션: 마우스 휠을 돌리면 클릭 휠이 실제로 회전하는 애니메이션이 발생하며, 상단 LCD 화면 속 앨범 커버가 GSAP Horizontal Scroll로 부드럽게 전환됩니다.

비주얼: 기기 본체의 메탈 질감과 LCD의 거친 픽셀 느낌을 살려, 디지털과 아날로그가 공존하는 QWER의 색깔을 표현합니다.

2. [Vinyl Odyssey] "The Growth Timeline" (추천 컨셉)
   UI: 화면 중앙에 거대한 LP 판이 앨범 커버와 함께 떠 있습니다.

인터랙션: 사용자가 스크롤하면 LP판이 물리 법칙에 따라 회전하며 옆으로 이동합니다. 특정 앨범에서 멈추면 해당 앨범의 테마 컬러(예: 마젠타, 블루)로 배경의 Mesh Gradient가 서서히 물듭니다.

호버 효과: 앨범에 마우스를 올리면 GSAP Flip 효과와 함께 LP 패키지에서 가사집(수록곡 카드)이 촤르륵 튀어나옵니다.

3. [Bento Grid Expansion] "Harmony Mosaic"
   UI: 앨범 아트워크들이 불규칙하지만 조화로운 그리드 형태로 배치됩니다.

인터랙션: 특정 앨범 카드를 클릭하면 Shared Layout Transition을 통해 해당 카드가 화면 전체로 확장되며(Zoom-in), 배경으로 깔려있던 수록곡 리스트들이 Stagger 효과와 함께 나타납니다.

비주얼: "In a million noises"라는 문구처럼, 흩어져 있던 소음들이 하나의 하모니로 모이는 시각적 경험을 선사합니다.

4. [Interactive Concert Stage] "The Live Archive"
   사용자가 마치 콘서트장에 와 있는 듯한 공간감을 주는 새로운 컨셉입니다.

UI: 어두운 스테이지 위에 악기 세팅과 함께 앨범들이 조명을 받으며 배치되어 있습니다.

인터랙션: GSAP ScrollTrigger를 사용하여 카메라 시점이 무대 위를 이동하는 듯한(Parallax) 느낌을 줍니다. 앨범을 클릭하면 해당 곡의 '응원 소리'가 배경음으로 작게 깔리며 상세 페이지로 유도합니다.

비주얼: 실제 공연장의 조명 효과와 안개 효과를 CSS Filter와 Canvas로 구현하여 몰입감을 높입니다.
